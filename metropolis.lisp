...
(defun xyz-to-lab (x y z &key (white-x 0.3127) (white-y 0.3290)
		              (white-luminance 1.0))
  (flet ((f (x)
	   (if (> x (expt 6/29 3))
	       (expt x 1/3)
...
(defun lab-to-xyz (L* a* b* &key (white-x 0.3127) (white-y 0.3290)
		                 (white-luminance 1.0))
  (flet ((f (x)
	   (if (> x 6/29)
	       (expt x 3)
...
(defun xyz-to-uv (x y z)
  (let ((a (+ x (* 15 y) (* 3 z))))
    (values (/ (* 4 x) a) (/ (* 9 y) a))))

(defun xyz-to-luv (x y z &key (white-x 0.3127) (white-y 0.3290)
		              (white-luminance 1.0))
  (flet ((f (x)
	   (if (<= x (expt 6/29 3))
	       (* (expt 29/3 3) x)
    ...
    (let* ((wx (* white-luminance (/ white-x white-y)))
	   ...
	   (wz (* white-luminance (/ (- 1 white-x white-y) white-y)))
	   (L* (f (/ y wy))))
      (multiple-value-bind (u v) (xyz-to-uv x y z)
	(multiple-value-bind (wu wv) (xyz-to-uv wx wy wz)
	  (let ((u* (* 13 L* (- u wu)))
		(v* (* 13 L* (- v wv))))
	    (values L* u* v*)))))))

(defun luv-to-xyz (L* u* v* &key (white-x 0.3127) (white-y 0.3290)
		                 (white-luminance 1.0))
  (flet ((u (x y z)
	   (/ (* 4 x) (+ x (* 15 y) (* 3 z))))
	 (v (x y z)
  ...
  (with-window (:width width :height height)
    (labels ((window-x (x)
	     ...
	     (draw-cross (x y &optional (size 5))
	       ...
	       (draw-line x (- y size) x (+ y size 1))))
      (when (and white-x white-y)
	(multiple-value-bind (l a b) (xyz-to-lab
				      (/ white-x white-y)
				      1.0
      ...
      (loop for nm from 400 to 700 by .1 do
	   (let ((x (wavelength-response :x nm))
		 (y (wavelength-response :y nm))
		 (z (wavelength-response :z nm)))
	     (multiple-value-bind (l a b) (xyz-to-lab (/ x y) 1 (/ z y))
      ...
      (let ((x1 (wavelength-response :x 360))
	    ...
	    (y2 (wavelength-response :y 830))
	    (z2 (wavelength-response :z 830)))
	(multiple-value-bind (l1 a1 b1) (xyz-to-lab (/ x1 y1) 1 (/ z1 y1))
	  (multiple-value-bind (l2 a2 b2) (xyz-to-lab (/ x2 y2) 1 (/ z2 y2))
...
(defun draw-cie-xy ()
  (let* ((width 600)
	...
	(loop for y from 0 below height
	      for y* = (- .85 (/ y height))
	      for x1 = (round (* width (x-min y*)))
	      for x2 = (round (* width (x-max y*))) do
	      (draw-line 0 y x1 y)
...
(defun draw-cie-uv ()
  (let ((width 400)
	...
	(max -1d10)
	(background-grey 0.5))
    (multiple-value-bind (white-u white-v) (xyz-to-uv wx wy (- 1 wx wy))
      (labels ((wavelength-u (nm)
		 (let ((x (wavelength-response :x nm))
		       (y (wavelength-response :y nm))
		       (z (wavelength-response :z nm)))
		   (nth-value 0 (xyz-to-uv x y z))))
	       (wavelength-v (nm)
		 (let ((x (wavelength-response :x nm))
		       (y (wavelength-response :y nm))
		       (z (wavelength-response :z nm)))
		   (nth-value 1 (xyz-to-uv x y z))))
	       (find-u (v fn nm1 nm2)
		 (loop for nm = (/ (+ nm1 nm2) 2)
		       until (< (- nm2 nm1) .00001) do
		       (if (funcall fn (wavelength-v nm) v)
			   (setf nm1 nm)
			   (setf nm2 nm))
		       finally (return (wavelength-u nm))))
	       (u-min (v)
		 (find-u v #'< 360 525))
	       (u-max (v)
		 (if (>= v (wavelength-v 700))
		     (find-u v #'> 525 700)
		     (let ((u1 (wavelength-u 360))
			   (v1 (wavelength-v 360))
			   (u2 (wavelength-u 700))
			   (v2 (wavelength-v 700)))
		       (+ (* (- u2 u1) (/ (- v v1) (- v2 v1))) u1)))))
	(with-window (:width width :height height)
	  (loop for y from 0 below height
		for v = (lerp 0.6 0 (/ y height))
		for u1 = (u-min v)
		for u2 = (u-max v) do
		(format t "~10F ~10F~%" u1 u2)
		(draw-line 1 2 3 4)
		(loop for x from 0 to width
		      for u = (lerp 0 0.6 (/ x width))
		      when (<= u1 u u2) do
		      (multiple-value-bind (r g b)
			  (let* ((L* 50)
				 (u* (* 13 L* (- u white-u)))
				 (v* (* 13 L* (- v white-v))))
			    (multiple-value-call #'xyz-to-rgb
			      (luv-to-xyz L* u* v*)))
			(setf min (min min r g b))
			(setf max (max max r g b))
			;(with-preserved-luminosity (r g b)
			  (incf r background-grey)
			  (incf g background-grey)
			  (incf b background-grey);)
			#+(or)
			(with-preserved-luminosity (r g b)
			  (setf r (max r 0))
			  (setf g (max g 0))
  ...
  (let ((width 600/2)
    ...
    (with-window (:width width :height height)
      (loop for y below height
	    ...
	    (flet ((foo (fn)
		     (loop for x below width
			   for x2 = (lerp blue red (mod (- (/ x width) w) 1))
			   for y2 = (lerp blue red (mod (+ (/ x width) w) 1)) do
